...
TalkMessagMany::TalkMessagMany(QWidget *parent,QString iconid_self,QString iconid_other,QString id_self,
                               QString id_other) :
    QDialog (parent)
{
    QFont tm_font ;
    tm_font.setFamily("MicrosoftYaHei");
    tm_font.setPixelSize(16);
    this->setFont(tm_font);
    m_leftPixmap = QPixmap(QString(":/portrait/no%1.jpg").arg(iconid_other));
    m_rightPixmap = QPixmap(QString(":/portrait/no%1.jpg").arg(iconid_self));
    m_id_self=id_self;
    m_id_other=id_other;

}

TalkMessagMany::~TalkMessagMany()
{
}


void TalkMessagMany::sendText(QString text, QString time, QSize size, msg_type msgtype)
{//初始化对话框中信息的文本样式
    m_msg = text;
    m_type = msgtype;
    m_sendtime = time;
    m_nowTime = QDateTime::fromTime_t(time.toInt()).toString("hh:mm");
    m_size = size;
    this->update();
}

void TalkMessagMany::paintEvent(QPaintEvent *event)
{//画气泡式对话框
    Q_UNUSED(event);
    ...
    if( m_type== msg_type::user_other) { // 用户
        //头像
//        painter.drawRoundedRect(m_iconLeftRect,m_iconLeftRect.width(),m_iconLeftRect.height());
        ...
        QPen penId;
        penId.setColor(Qt::GlobalColor::darkGray);
        painter.setPen(penId);
        option.setWrapMode(QTextOption::WrapAtWordBoundaryOrAnywhere);
        QFont idfont;
        idfont.setPixelSize(15);
        idfont.setFamily("MicrosoftYaHei");
        painter.setFont(idfont);
        painter.drawText(m_idLeftRect,m_id_other,option);
    }
    else if(m_type == msg_type::user_self) { // 自己
//        painter.drawRoundedRect(m_iconRightRect,m_iconRightRect.width(),m_iconRightRect.height());
        ...
        idfont.setFamily("MicrosoftYaHei");
        painter.setFont(idfont);
        painter.drawText(m_idRightRect,m_id_self,option);

    }
    else if(m_type == msg_type::user_time) { // 时间
        QPen penText;
        ...
        painter.drawText(this->rect(),m_nowTime,option);
    }
}

QSize TalkMessagMany::fontRect(QString str)
{//确定消息的位置
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
    ...
    m_iconRightRect = QRect(this->width() - iconSpaceW - iconWH-170, iconTMPH, iconWH, iconWH);

    QSize size = getMessageSize(m_msg); // 整个的size
    QSize size_id_self = getMessageSize(m_id_self); // 自己id的size
    QSize size_id_other= getMessageSize(m_id_other);//其他人id的size

    int hei = size.height() < minHei ? minHei : size.height();

    m_sanjiaoLeftRect = QRect(iconWH+iconSpaceW+iconRectW, m_lineHeight/2, sanJiaoW, hei - m_lineHeight);
    m_sanjiaoRightRect = QRect(this->width() - iconRectW - iconWH - iconSpaceW - sanJiaoW-170, m_lineHeight/2, sanJiaoW, hei - m_lineHeight);
    ...
    if(size.width() < (m_textWidth+m_spaceWid)) {
        ...
        m_kuangRightRect.setRect(this->width() - size.width() + m_spaceWid - 2*textSpaceRect - iconWH - iconSpaceW - iconRectW - sanJiaoW-170,
        ...
        m_kuangRightRect.setRect(iconWH + kuangTMP + iconSpaceW + iconRectW - sanJiaoW-170, m_lineHeight/4*3+10, m_kuangWidth, hei-m_lineHeight);
    }
    ...
    m_idLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), m_iconRightRect.y()-15, iconWH+50, iconWH);
    m_idRightRect.setRect(m_iconRightRect.x()-size_id_self.width()+160, m_iconRightRect.y()-15, iconWH+50, iconWH);

    return QSize(size.width(), hei+15);
}

QSize TalkMessagMany::getMessageSize(QString src)
{//根据消息内容确定框的大小
