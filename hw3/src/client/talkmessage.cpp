...
talkmessage::talkmessage(QWidget *parent,QString iconid_other,QString iconid_self) :
    QWidget(parent)
{
    QFont tm_font ;
...
void talkmessage::sendText(QString text, QString time, QSize size, msg_type msgtype)
{//初始化对话框中信息的文本样式
    m_msg = text;
    m_type = msgtype;
    m_sendtime = time;
...
void talkmessage::paintEvent(QPaintEvent *event)
{//画气泡式对话框
    Q_UNUSED(event);
...
QSize talkmessage::fontRect(QString str)
{//确定消息的位置
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
    ...
    m_iconRightRect = QRect(this->width() - iconSpaceW - iconWH - 7, iconTMPH, iconWH, iconWH);
...
QSize talkmessage::getMessageSize(QString src)
{//根据消息内容确定框的大小
