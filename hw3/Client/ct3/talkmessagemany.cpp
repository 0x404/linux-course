...
//    te_font.setLetterSpacing(QFont::AbsoluteSpacing, 0);             //设置字间距为3像素 //设置字间距像素值
    ...
    m_leftPixmap = QPixmap(QString(":/new/prefix1/%1.png").arg(iconid_other));
    m_rightPixmap = QPixmap(QString(":/new/prefix1/%1.png").arg(iconid_self));
    m_id_self=id_self;
    m_id_other=id_other;
...
{//确定消息的位置
    ...
    if(size.width() < (m_textWidth+m_spaceWid)) {
        ...
        m_kuangRightRect.setRect(this->width() - size.width() + m_spaceWid - 2*textSpaceRect - iconWH - iconSpaceW - iconRectW - sanJiaoW-170,
                                 m_lineHeight/4*3+15, size.width()-m_spaceWid+2*textSpaceRect, hei-m_lineHeight);
    } else {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), m_lineHeight/4*3+10, m_kuangWidth, hei-m_lineHeight);
        m_kuangRightRect.setRect(iconWH + kuangTMP + iconSpaceW + iconRectW - sanJiaoW-170, m_lineHeight/4*3+15, m_kuangWidth, hei-m_lineHeight);
    }
    ...
    m_idRightRect.setRect(m_iconRightRect.x()-size_id_self.width()+160, m_iconRightRect.y()-10, iconWH+50, iconWH);
